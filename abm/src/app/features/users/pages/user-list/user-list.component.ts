...
export class UserListComponent implements OnInit {
  ...
  getEmptyRows(): any[] {
    const total = this.filteredUsers.length;      // total de usuarios filtrados
    const current = this.paginateUsers.length;    // usuarios en la página actual
    const maxPerPage = this.pageSize;

    // Solo mostrar filas vacías si hay posibilidad de cambiar de página
    if (total > maxPerPage && current < maxPerPage) {
      const diff = maxPerPage - current;
      return Array(diff);
    }
